...
def putPolyomino(polyomino, p, qr, afterQr, x, y, d, reflection):
    ...
    tileQr = [[-999 for _ in range(SIZE)] for _ in range(SIZE)]
        ...
        for j in range(FRAME_SIZE):
            ...
            for s in range(3):
                for t in range(3):
                    ...
                    tileQr[3 * dx + s][3 * dy + t] = movedQr[3 * i + s][3 * j + t]

    printQr(sys.stdout, tileQr, tileQr)
    print()
    return True
...
for id in ids0 + ids1000 + ids2000 + ids3000 + ids4000 + ids5000:
    for d in range(4):
        for p in ["A", "B"]:
            if not (id == 2959 and d == 0 and p == "B"):
                continue
