...
class HandUtil:
    ...
    def getHandNumber(self) -> int:
        ...
        以下のように5進数の12桁で表し、その数字を返す
        CCHHHHHHHHHP

        C部:
            正規系にした時の枚(1->0, 4->1, 7->2, 10->3, 13->4)
            頭の情報(雀頭接続順子が別にあり->0, 雀頭が別にあり->1, その他->2)
        H部:
            各数字の牌の枚数を保持する部で、4からその枚数を弾いた数を持たせる。
        P部:
            位置を管理する(長さ7以下->0, 長さ8で左接地->1, 長さ8で右接地->2, 長さ9->3)
