...
def play_one_game(policies: list, seed: int, verbose: bool = False):
    """1ゲーム実行して (rewards, winner) を返す。"""
...
def run_tournament(policies: list, num_games: int, base_seed: int = 0):
    ...
    for g in range(num_games):
        seed = base_seed + g
        rewards, winner = play_one_game(policies, seed)
    ...
    for i in range(NUM_PLAYERS):
        ...
        print(f"  P{i}: avg_points={avg:.2f}, wins={wins[i]} ({winrate:.1f}%)")
